...
import { ArrowLeft, Flame, Users, Zap, Coins, ChevronRight, Sparkles, Crown } from 'lucide-react-native';
...
export default function ClubInfoDefault(props: ClubInfoProps) {
  const router = useRouter();
  const { user, settings, loading, theme, isEnabled, pointsRate, referralPoints, streak, multiplier, progressWidth } = props;

  return (
    <SafeAreaView className="flex-1 bg-black">
       <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
          <View className="relative">
             <LinearGradient colors={['#1C1408', '#0F0A05', '#000000']} className="pt-14 pb-20 px-6">
                ...
                <Text className="text-white text-[52px] font-black tracking-[-3px] leading-none">DOMINA{"\n"}EL CLUB</Text>
                <Text className="text-white/70 text-lg mt-6 max-w-xs">Donde cada peso se convierte en poder. Cada misión en estatus.</Text>
             </LinearGradient>
          ...
          <View className="px-6 -mt-8 mb-10 z-10">
             <View className="bg-[#111] border border-white/10 rounded-3xl p-8 shadow-2xl">
                ...
                <View className="mt-8 h-2 bg-white/5 rounded-full overflow-hidden border border-white/5">
                   <LinearGradient colors={[theme.secondary, theme.primary]} start={{ x: 0, y: 0.5 }} end={{ x: 1, y: 0.5 }} style={{ width: progressWidth as any, height: 8, borderRadius: 10 }} />
                </View>
          ...
          <View className="px-6 mb-12">
             <View className="bg-[#111] border border-[#E8C670]/10 rounded-3xl p-8 overflow-hidden">
                ...
                <View className="mt-8 pt-6 border-t border-white/5">
                   <Text className="text-center text-white/40 text-[11px] font-medium leading-relaxed uppercase tracking-wider">1 peso = {pointsRate} puntos • Crédito inmediato en tu cuenta VIP</Text>
                </View>
             </View>
          </View>
          ...
          <View className="px-6 mb-12">
             ...
             <View className="gap-y-4">
                {isEnabled('enable_referrals') && (
                   <MissionCard icon={Users} color="#3B82F6" title="Invita y conquista" description="Trae amigos al club" reward={`${referralPoints}`} accent onPress={() => router.push('/(tabs)/profile')} />
                )}
                <MissionCard icon={Sparkles} color="#10B981" title="Check-in en la puerta" description="Escanea el QR de entrada al llegar" onPress={() => router.push('/(tabs)/scanner')} />
                <MissionCard icon={Coins} color={theme.secondary} title="Escanea en barra" description="Usa el QR del POSNET" onPress={() => router.push('/(tabs)/scanner')} />
             </View>
          </View>

          <View className="px-6 pb-16">
             <TouchableOpacity onPress={() => router.back()} activeOpacity={0.95} className="h-20 rounded-3xl overflow-hidden border border-[#E8C670]/30 shadow-2xl">
                <LinearGradient colors={[theme.secondary, theme.secondaryDark]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} className="flex-1 items-center justify-center">
